...
# 1st Task -> sorting by price, asc
def get_price(item):
    return item[1][4]


car_data_sorted_by_price = dict(sorted(car_data.items(), key=get_price))
print(car_data_sorted_by_price)
...
car_data_filter_general = {
    ...
    for car_brand, data in car_data_sorted_by_price.items()
    if (
            data[1] >= search_criteria[0] and
            data[2] >= search_criteria[1] and
